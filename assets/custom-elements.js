...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!customElements.get("click-counter-button")) {
    try {
      ...
      console.warn(
        "⚠️ Erreur lors de l'enregistrement de 'click-counter-button':",
        error
      );
    }
  }
  ...
  if (!customElements.get("hello-message")) {
    try {
      ...
      console.warn(
        "⚠️ Erreur lors de l'enregistrement de 'hello-message':",
        error
      );
    }
  }
  ...
  if (!customElements.get("colored-box")) {
    try {
      ...
      console.warn(
        "⚠️ Erreur lors de l'enregistrement de 'colored-box':",
        error
      );
    }
  }
  ...
  if (!customElements.get("privacy-banner-element")) {
    try {
      class PrivacyBannerElement extends HTMLElement {
        ...
        connectedCallback() {
          this.innerHTML = `<div class="privacy-banner-container" style="background:#816C58; color:white; padding: 20px; text-align: center; position: relative;">
      ...
      console.warn(
        "⚠️ Erreur lors de l'enregistrement de 'privacy-banner-element':",
        error
      );
      ...
      try {
        ...
        console.log(
          "✅ Fallback custom element 'shopiweb-privacy-banner' registered"
        );
      } catch (fallbackError) {
        console.error(
          "❌ Impossible de définir le custom element fallback:",
          fallbackError
        );
      }
    }
