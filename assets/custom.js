...
document.addEventListener("DOMContentLoaded", function () {
  document
    .querySelectorAll('.color-swatches input[type="radio"][data-color-key]')
    .forEach(function (input) {
      input.addEventListener("change", function () {
        var optionPosition =
          input.getAttribute("data-option-position") ||
          input.closest("input").getAttribute("data-option-position");
        var colorKey = input.getAttribute("data-color-key");
        var labelSpan = input
          .closest(".product-option-wrapper")
          .querySelector(".js-option-selected");
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.querySelectorAll(".animated-stories-link").forEach(function (el) {
    el.addEventListener("click", function (e) {
      ...
      if (isMobile) {
        ...
        e.preventDefault();
        document
          .querySelectorAll(".tooltip-bubble.visible")
  ...
  try {
    fetch(
      "https://services.shopiweb.fr/api/licenses/get_by_domain/f6d72e-0f.myshopify.com/premium"
    )
      .then(response => {
      ...
      .catch(error => {
        console.warn(
          "⚠️ Validation de licence échouée : fonctionnement limité.",
          error
        );
      });
  } catch (error) {
    console.warn(
      "❌ Erreur critique lors du fetch de licence Shopiweb :",
      error
    );
  }
  ...
  function syncOptionLabelFromInput(input) {
    ...
    const labelEl = wrapper.querySelector(
      ".color-swatches-title .js-option-selected, .size-buttons-title .js-option-selected, .js-option-selected"
    );
    if (!labelEl) return;
    ...
    if (li) {
      ...
      if (swatchLabelEl) {
        swatchLabel =
          swatchLabelEl.getAttribute("title") ||
          swatchLabelEl.textContent.trim();
      }
    }
    if (!swatchLabel) {
