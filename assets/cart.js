...
window.updateCartContents = async response => {
  try {
    ...
    document.querySelectorAll(".cart-count-badge").forEach(badge => {
      badge.textContent =
        newDocument.querySelector(".cart-count-badge")?.textContent || "0";
    ...
    if (response.ok) {
      ...
      if (response.url.includes("add.js")) {
        ...
        if (data.items?.length > 1) {
          ...
          offcanvasCart
            ?.querySelector("[data-alert-item-added]")
            ?.removeAttribute("hidden");
        }
      } else {
        offcanvasCart
...
const initializeCartGoalFreeProduct = async () => {
  ...
  if (
    !cartGoal ||
    !cartGoal.dataset.goalCompleted ||
    !cartGoal.dataset.freeProductHandle
  )
    return;
  ...
  let isAlreadyInCart = [
    ...document.querySelectorAll("[data-cart-line-item]")
  ].some(e => e.dataset.productHandle === handle);
...
window.fetchShippingRates = async btn => {
  ...
  ["danger", "warning", "success"].forEach(type => {
    ...
    modal
      .querySelector(`#shipping-rates-modal-alert-${type}`)
