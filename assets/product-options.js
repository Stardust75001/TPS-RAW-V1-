...
function handleProductOptionChange(e) {
  ...
  if (matchedVariant) {
    const priceEl = form.querySelector(
    ...
    const imageEl = form.querySelector(
      `.product-image[data-product-id="${productId}"]`
    );
    ...
    if (atcBtn) {
      ...
      atcBtn.textContent = matchedVariant.available
        ? "Ajouter au panier"
        : "Indisponible";
